...
def get_nuggt(user_input, output_format, variables):
    tools = []
    ...
    for variable in variables:
        ...
        if type == "text":
            if choice not in value_dict.keys():
                temp = st.text_input(f"Enter value for {choice}: ")
            ...
            if choice not in value_dict.keys():
                uploaded_file = st.file_uploader(f"Upload {choice}") 
                replace_string = "{" + variable + "}"
...
def main():
    ...
    if user_input and output_format:
        ...
        nuggt, value_dict = get_nuggt(user_input, output_format, variables)
        print(nuggt)
